...
const parsePersona = (
  raw: unknown
): 'SECURITY_ANALYST' | 'COMPLIANCE_MANAGER' | 'EXECUTIVE' | 'ADMINISTRATOR' | null => {
...
const shouldBypassEmailVerification = (): boolean => {
  return (
    ...
    process.env.ENABLE_DEV_EMAIL_BYPASS === 'true'
  );
};
...
class AuthController {
    ...
    try {
      ...
      if (!user) {
        return res.status(404).json({
          success: false,
          message:
            'User not found in database. Please initialize the user.',
      ...
      const activeMembership = desiredOrgId
        ? user.memberships?.find(
            (m) => m.organizationId === desiredOrgId
          ) || user.memberships?.[0]
        : undefined;

      const orgName =
        activeMembership?.organization?.name || user.organization;
          ...
          if (email) {
            ...
            if (user) {
              ...
              if (shouldLock) {
                ...
                if (
                  typeof emailService.sendAccountLockedEmail ===
                  'function'
                ) {
  ...
  async initUser(
    ...
    if (!firebaseId) {
      return res.status(401).json({
        message:
          'Firebase ID not found on authenticated user.',
      });
    }
    ...
    try {
      ...
      if (!firebaseUser.email) {
        return res.status(400).json({
          message:
            'User email not found in Firebase token.',
        });
      }
      ...
      if (user) {
        ...
        return res.status(200).json({
          message:
            'User already initialized (by email).',
          user,
        });
      }
      ...
      const requestedNameRaw =
        typeof (req.body as any)?.name === 'string'
          ? String((req.body as any).name).trim()
          : '';

      const requestedName =
        requestedNameRaw &&
        requestedNameRaw.length <= 80
          ? requestedNameRaw
          : '';

      const requestedOrgNameRaw =
        typeof (req.body as any)?.organizationName ===
        'string'
          ? String(
              (req.body as any).organizationName
            ).trim()
          : '';

      const requestedPersona = parsePersona(
        (req.body as any)?.persona
      );
      ...
      const fallbackOrgName = (() => {
        ...
        const safeBase =
          base.length > 60 ? base.slice(0, 60) : base;
      ...
      try {
        ...
        // If multiple init requests race, the second create
        // can hit unique constraints.
        // Treat init as idempotent by fetching the existing
        // user and responding accordingly.
        if (error?.code === 'P2002') {
          const existingByFirebaseId =
      ...
      if (shouldBypassEmailVerification()) {
        ...
        await emailer.sendVerificationEmail(
          user.email,
          token
        );
      }

      // IMPORTANT: Do NOT treat a newly-created unverified user
      // as authenticated.
  ...
  async resetPassword(
    ...
    try {
      ...
      if (
        PasswordValidator.checkPasswordHistory(
          newPassword,
      ...
      const hashedPassword =
        PasswordValidator.hashPassword(newPassword);
  ...
  async changePassword(
    ...
    try {
      ...
      // Note: We can't directly verify the password,
      // so we'll update it and let Firebase handle validation
      ...
      // Update password history
      const hashedPassword =
        PasswordValidator.hashPassword(
          newPassword
        );
